    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      <header className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 leading-tight">Fu&szlig;ball-Vereinshistorie</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex gap-1 bg-gray-100 rounded-xl p-1 mb-6 w-fit">
          <button
            ...
            className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${mode === "clubs" ? "bg-white shadow text-gray-900" : "text-gray-500 hover:text-gray-700"}`}
          >
            &#9878; Vereine vergleichen
          </button>
          <button
            onClick={() => setMode("player")}
            className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${mode === "player" ? "bg-white shadow text-gray-900" : "text-gray-500 hover:text-gray-700"}`}
          >
            &#128100; Spieler suchen
          </button>
        </div>
        ...
        {mode === "player" && (
          <>
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-4">Spieler suchen</h2>
              <PlayerSearch onSelect={setSelectedPlayer} />
              <p className="text-xs text-gray-400 text-center mt-3">
                Mindestens 2 Zeichen eingeben &mdash; Autocomplete aus Wikidata
              </p>
            </div>
            ...
            {!selectedPlayer && (
              <div className="text-center py-16 text-gray-400">
                ...
                <p className="text-base font-medium text-gray-500 mb-1">Spieler suchen und Karriere entdecken</p>
                <p className="text-sm">Gib einen Spielernamen ein, z.&nbsp;B. &bdquo;Lewandowski&ldquo; oder &bdquo;Kahn&ldquo;.</p>
              </div>
            )}
          </>
        )}

        {/* ── CLUB COMPARISON MODE ── */}
        {mode === "clubs" && (
          <>
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-4">Vereine ausw&auml;hlen</h2>
              <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-end">
                <ClubSearch label="Verein A" accentColor="blue" value={clubA} onChange={setClubA} />
                <div className="flex items-center justify-center sm:pb-3">
                  <div className="text-gray-300 font-bold text-lg">vs</div>
                </div>
                <ClubSearch label="Verein B" accentColor="yellow" value={clubB} onChange={setClubB} />
                <button
            ...
            {loading && (
              <div className="text-center py-20">
                <div className="inline-flex flex-col items-center gap-4">
                  ...
                  <p className="text-gray-400 text-xs">Dies kann bei gro&szlig;en Vereinen einen Moment dauern.</p>
                </div>
              </div>
            )}
            ...
            {result && !loading && (
              <>
                <div className="mb-4">
                  <FilterBar filters={filters} onChange={setFilters} hasCommon={result.commonPlayers.length > 0} />
                </div>
                ...
                <div className="flex gap-1 bg-gray-100 rounded-xl p-1 mb-4 overflow-x-auto">
                  {tabs.map((tab) => (
                    <button
                      ...
                      className={`flex-1 min-w-max px-3 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === tab.key ? "bg-white shadow text-gray-900" : "text-gray-500 hover:text-gray-700"}`}
                    >
                      {tab.label}
                      {tab.count !== undefined && (
                ...
                {activeTab === "both" && (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div className="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden flex flex-col" style={{ maxHeight: "75vh" }}>
                      ...
                      <PlayerList entries={result.playersA} commonIds={commonIds} accentColor="blue" filters={filters} clubName={result.clubA.name} />
                    </div>
                    <div className="bg-white rounded-2xl border border-yellow-100 shadow-sm overflow-hidden flex flex-col" style={{ maxHeight: "75vh" }}>
                      ...
                      <PlayerList entries={result.playersB} commonIds={commonIds} accentColor="yellow" filters={filters} clubName={result.clubB.name} />
                    </div>
                  </div>
                )}
                ...
                {activeTab === "common" && (
                  <CommonPlayersPanel commonPlayers={result.commonPlayers} clubAName={result.clubA.name} clubBName={result.clubB.name} />
                )}
              </>
            )}
            ...
            {!result && !loading && !error && (
              <div className="text-center py-20 text-gray-400">
                ...
                <p className="text-lg font-medium text-gray-500 mb-1">Vereine ausw&auml;hlen und vergleichen</p>
                <p className="text-sm">Gib zwei Vereinsnamen ein und klicke auf &bdquo;Vergleichen&ldquo;.</p>
                <div className="mt-6 flex flex-wrap justify-center gap-2">
                  {["FC Bayern M&uuml;nchen vs Borussia Dortmund", "Real Madrid vs FC Barcelona", "Arsenal vs Chelsea"].map((example) => (
                    <span key={example} className="text-xs bg-gray-100 text-gray-500 px-3 py-1.5 rounded-full" dangerouslySetInnerHTML={{ __html: example }} />
                  ))}
                </div>
              </div>
            )}
